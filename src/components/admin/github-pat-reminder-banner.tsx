...
export function ReminderBanner({
  ...
  const border = unset
    ? "border-[#ECFF95]/30 bg-[#ECFF95]/[0.06]"
    : overdue
      ? "border-red-500/40 bg-red-500/[0.06]"
      : daysLeft !== null && daysLeft <= 2
        ? "border-[#ECFF95]/35 bg-[#ECFF95]/[0.04]"
  ...
  return (
    <div className={`mb-8 p-4 border ${border} flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3`}>
      <div className="flex items-start gap-3 min-w-0">
        <span className="shrink-0 text-[#ECFF95]/70 mt-0.5">{icon ?? <KeyRound size={18} />}</span>
      ...
      <Link
        href="/admin/security"
        className="shrink-0 inline-flex items-center justify-center px-4 py-2.5 bg-[#ECFF95] text-[#121316] font-['Anton'] text-[11px] tracking-[0.1em] hover:bg-white transition-colors"
      >
        MANAGE REMINDER
      </Link>
    </div>
